 ...
 */
class SQLController implements GroupProvider, GroupPublisher, MemberProvider<SocietyMember>, MemberPublisher<SocietyMember> {
    ...
    public SQLController(SQLQueries queries,
                         ...
                         PlayerProvider playerProvider, MemberFactory<SocietyMember> memberFactory) {
    ...
    private SocietyMember evaluateMember(UUID uuid, Group predefined, Result<MembersRecord> input) {
        ...
        try {
            ...
            if (rawSociety != null && rawSociety.length == UUIDGen.UUID_LENGTH) {
                ...
                if (predefined == null) {
                    try {
                        predefined = getGroup(UUIDGen.toUUID(rawSociety), member).get();
        ...
        for (Record1<byte[]> member : query.fetch()) {
            try {
                ...
                if (predefined != null && predefined.getUUID().equals(memberUUID)) {
                    ...
                    memberToAdd = getMember(memberUUID, group).get();
                }
